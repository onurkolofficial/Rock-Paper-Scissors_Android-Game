...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("join_matchmaking", (data: { name: string, skin?: string }) => {
    ...
    if (!joinedRoom) {
      ...
      rooms[newRoomId] = {
        id: newRoomId,
        players: {
          [socket.id]: { socketId: socket.id, name: data.name || 'Guest', move: null, score: 0, skin: data.skin || 'default' }
        },
        status: 'waiting',
        round: 1,
        draws: 0,
  ...
  socket.on("join_private_room", (data: { name: string, roomId: string, skin?: string }) => {
    ...
    if (room && room.status === 'waiting' && room.isPrivate && Object.keys(room.players).length === 1) {
      room.players[socket.id] = { socketId: socket.id, name: data.name || 'Guest', move: null, score: 0, skin: data.skin || 'default' };
      room.status = 'playing';
      room.round = 1;
      room.draws = 0;
      playerRoomMap.set(socket.id, roomId);
